...
/**
 * Script to scrape 50 recent job listings from South African job sites
 * using Firecrawl API and add them to database/jobs.json
 */
...
function transformToJobFormat(firecrawlJob: FirecrawlJobListing, index: number): Job {
  ...
  // Generate unique ID
  const id = `firecrawl-${Date.now()}-${index}`;
...
async function fetchJobsFromFirecrawl(url: string, count: number): Promise<FirecrawlJobListing[]> {
  ...
  const prompt = `Gather ${count} of the most recent job listings from ${url} in South Africa across various industries with all necessary data for Google Jobs Schema.`;
  ...
  try {
    ...
    if (!data || !data.job_listings || !Array.isArray(data.job_listings)) {
      console.warn(`  No job listings found from ${url}`);
      return [];
    }
...
async function main() {
  ...
  for (const site of SA_JOB_SITES) {
    ...
    try {
      ...
      if (allJobs.length >= TARGET_JOBS) {
        console.log(`✅ Reached target of ${TARGET_JOBS} jobs\n`);
        break;
      }
    } catch (error) {
  ...
  console.log(`\n📊 Total jobs scraped: ${finalJobs.length}`);

  if (finalJobs.length === 0) {
    console.error('❌ No jobs were scraped. Exiting without updating database.');
    process.exit(1);
  }
  ...
  console.log(`✨ Adding ${newJobs.length} new jobs (${finalJobs.length - newJobs.length} duplicates skipped)`);

  // Merge and save
  const allJobsToSave = [...existingJobs, ...newJobs];
  writeFileSync(dbPath, JSON.stringify(allJobsToSave, null, 2), 'utf-8');

  console.log(`\n✅ Successfully updated database with ${allJobsToSave.length} total jobs`);
